...
<template>
<MkModal ref="modal" v-slot="{ type }" :preferType="deviceKind === 'smartphone' ? 'drawer' : 'dialog'" :backButton="() => withCloseButton ? emit('close') : emit('esc')" @click="onBgClick" @closed="emit('closed')" @esc="emit('esc')">
