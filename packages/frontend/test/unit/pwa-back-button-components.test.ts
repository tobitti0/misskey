...
describe('PWA back integration with windows and modals', () => {
	...
	let windowHistory: DeckWindowHistory | null;
	let historyListeners: AbortController;
	...
	beforeEach(() => {
		...
		showExit.mockClear();
		windowHistory = null;
		historyListeners = new AbortController();
	...
	test('a replaced route is not added to the page window Back history', async () => {
		...
		window.history.back();
		await vi.waitFor(() => expect(router.getCurrentFullPath()).toBe('/posts/1'));
		window.history.back();
	...
	test('closing a lightbox in the PWA does not change browser history or navigate the window', async () => {
		...
		expect(routers[0].getCurrentFullPath()).toBe('/posts/2');
		window.history.back();
		await vi.waitFor(() => expect(routers[0].getCurrentFullPath()).toBe('/posts/1'));
	});

	function enableWindowHistory(openWindow: (state: DeckHistoryWindow) => void = () => {}) {
		window.history.replaceState(null, '', '/');
		windowHistory = new DeckWindowHistory(window.history, '/', openWindow, hasWindows => backButton.setAtRoot(!hasWindows));
		window.addEventListener('popstate', event => {
			windowHistory!.restore(event.state, window.location.pathname + window.location.hash);
		}, { signal: historyListeners.signal });
	}

	function mountHistoryWindows() {
		let key = 0;
		const windows = ref<{ key: number; initialPath: string; historyState?: DeckHistoryWindow }[]>([
			{ key: key++, initialPath: '/posts/1' },
		]);
		enableWindowHistory(state => {
			windows.value.push({ key: key++, initialPath: state.paths.at(-1)!, historyState: state });
		});
		mountContent(() => h('div', windows.value.map(w => h(MkPageWindow, {
			...w,
			onClosed: () => { windows.value = windows.value.filter(item => item.key !== w.key); },
		}))));
	}

	test('PWA windows use browser Back/Forward and arm native exit only after the window closes', async () => {
		mountHistoryWindows();
		const pushState = vi.spyOn(window.history, 'pushState');
		const router = routers[0];
		router.pushByPath('/posts/2');
		...
		await vi.waitFor(() => expect(router.getCurrentFullPath()).toBe('/posts/2'));
		window.history.back();
		await vi.waitFor(() => expect(router.getCurrentFullPath()).toBe('/posts/1'));
		window.history.back();
		await vi.waitFor(() => expect(root.querySelector('.ti-x')).toBeNull());
		...
		window.history.forward();
		await vi.waitFor(() => expect(routers).toHaveLength(2));
		expect(routers[1].getCurrentFullPath()).toBe('/posts/1');
		expect(pushState).toHaveBeenCalledOnce();
	});

	test('X closes a page window immediately in history and Forward keeps it closed', async () => {
		mountHistoryWindows();
		const baseState = window.history.state;
		routers[0].pushByPath('/posts/2');
		await nextTick();
		root.querySelector<HTMLElement>('.ti-x')!.parentElement!.click();
		await vi.waitFor(() => expect(root.querySelector('.ti-x')).toBeNull());
		// The open entry and its navigation have both been rewound to the deck base.
		expect(window.history.state).not.toEqual(baseState);
		window.history.forward();
		await new Promise(resolve => window.setTimeout(resolve, 10));
		await nextTick();
		expect(routers).toHaveLength(1);
		expect(root.querySelector('.ti-x')).toBeNull();
	});
});
